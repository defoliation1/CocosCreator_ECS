...
export class ECSWorld {
    ...
    private _entityIdSeed: number = 1;

    /** 获取ComponentPool */
    public getComponentPool<T>(typeOrFunc: ComType | ECSComConstructor): ECSComponentPool<T> {
    ...
    public getComponent<T extends ECSComConstructor>(entity: EntityIndex, com: T): T {
    ...
    public addComponent<T extends ECSComConstructor>(entity: EntityIndex, com: T): T {
        if(!this._entities[entity]) return null;
        if(this.getComponent(entity, com)) {
            console.warn(`[ECSWorld]: addComponent error com is had`);
            return this.getComponent(entity, com);
        }
        let comPoolIdx = this._entities[entity].addComponent(com);
        return this.getComponentPool<T>(com).get(comPoolIdx)
    }

    public getSingletonComponent<T extends ECSComConstructor>(com: T): T {
...
export function GenFillterKey(accepts: ECSComConstructor[], rejects?: ECSComConstructor[]) {
    ...
    if(rejects && rejects.length > 0) {
        for(let i = 0; i < rejects.length; i++) {
            rejectTypes[i] = GetComConstructorType(accepts[i]);
        }
    }
